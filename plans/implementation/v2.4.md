# Plan 2.4 implementation — Modal runtime tidy-up

Implements `plans/releases/2.4.md`. Refined against the code as of plan 2.1 T5
...
2. The assert sites named in 2.4 §1.5 moved with 2.1. The live assert sites
...
3. The `POLARS_MAX_THREADS` question from 2.4 §2.6 is settled: setting the
...
main ──┬── A  chore/modal-runtime-tidy        logging cut + wrapper inline   (modal_runtime.py)
       ...
       F  docs/v2.4-closeout               [after A–E all merged]
```
...
| F | `docs: verify apps/training docs against 2.4 reality; mark plan implemented` |
...
the trivial wrappers. No logic change. Spec: `plans/releases/2.4.md` §2.1, §2.3.
...
`[runtime.modal]` via `modal_runtime_config`. Spec: `plans/releases/2.4.md` §2.3.
...
it from all three current sites. Spec: `plans/releases/2.4.md` §2.4, extended to cover
...
the identical message text. Spec: `plans/releases/2.4.md` §2.5, updated for the sites
...
Removes the dead `POLARS_MAX_THREADS` plumbing. The question 2.4 §2.6 left
...
- `PreprocessJob` loses the field `max_threads: int = 8`
  ...
  Acceptable by plan 2.4's own terms ("no behaviour that matters"); note it
...
# Branch F — `docs/v2.4-closeout`

Verifies documentation claims against post-2.4 reality and marks the plan
...
updating plans/releases/2.4.md itself.
...
returns nothing. Commit: `docs: verify apps/training docs against 2.4 reality; mark plan implemented`.
