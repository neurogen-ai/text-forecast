# Plan 2.5 implementation — Modal runtime tidy-up

Implements `plans/releases/2.5.md`. Refined against the code as of plan 2.1 T5
...
the trivial wrappers. No logic change. Spec: `plans/releases/2.5.md` §2.1, §2.3.
...
`[runtime.modal]` via `modal_runtime_config`. Spec: `plans/releases/2.5.md` §2.3.
...
it from all three current sites. Spec: `plans/releases/2.5.md` §2.4, extended to cover
...
the identical message text. Spec: `plans/releases/2.5.md` §2.5, updated for the sites
...
`.venv/bin/forecite preprocess --help 2>/dev/null | grep -c max-threads || true`
...
updating plans/releases/2.5.md itself.
