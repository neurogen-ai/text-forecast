...
class ModelConfig(BaseModel):
    ...
    n_forward_inference: PositiveInt
    n_forward_train: PositiveInt
    n_backward_train: int
...
class AbstractorLM2(nn.Module):
    ...
    def __init__(
        ...
        self.head_scorer = MultiHeadSelfAttn(
                config.embed_dim, 
                config.hidden_dim, 
                config.n_forward_train + config.n_backward_train, 
    ...
    def generate(self, prompt: Tensor, max_len: int):
        ...
        for i in range(max_len):
            ...
            next = tokens[:, self.config.n_backward_train:self.config.n_backward_train + self.config.n_forward_inference]
            out += next.tolist()
