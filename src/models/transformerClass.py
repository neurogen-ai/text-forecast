...
class MultiHeadSelfAttn(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor) -> Tensor:
        ...
        # mask is (B, T) key-padding; broadcast to (B, 1, 1, T) for SDPA.
        out = nn.functional.scaled_dot_product_attention(
            q, k, v, attn_mask=mask[:, None, None, :], dropout_p=self.dropout
        )
...
@component
class TransformerClass(nn.Module, Model[ModelConfig, TokenBatch, Output]):
    config = ModelConfig
