...
def _records_for(caplog, tracker, store_name):
    module_logger = logging.getLogger(type(tracker).__module__)
    return [
        r for r in caplog.records
        if r.name == module_logger.name and store_name in r.getMessage()
    ]
...
def test_first_empty_gather_warns_once(tracker, caplog):
    ...
    records = _records_for(caplog, tracker, "train_ids")
...
def test_fed_store_does_not_warn(tracker, caplog):
    ...
    assert _records_for(caplog, tracker, "train_ids") == []
...
def test_instances_latch_independently(tracker, caplog):
    ...
    records = _records_for(caplog, other, "val_ids")
    assert len(records) == 1
    assert records[0].levelno == logging.WARNING
